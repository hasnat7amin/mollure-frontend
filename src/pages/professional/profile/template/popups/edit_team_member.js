...
export default function EditTeamMember({ id, type, showModel, setShowModel, data }) {
  ...
  const setFormData = () => {
    data && setTeamMemberName(data.member ? data.member : '');
  ...
  const isFormValid = () => {
    ...
    return rows.every(row => row.categoryId && row.serviceIds.length > 0) &&
      teamMemberName.trim() !== '' &&
      bio.trim() !== '' &&
      (selectedImage !== null || profileImage !== null);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    selectedImage && data.append('image', selectedImage.file);
            ...
            <ClickAwayListener onClickAway={() => setShowModel(false)}>
              <div className="relative px-2 py-4 bg-white rounded-lg shadow-lg w-full">
                ...
                <div className="px-5 pt-9">
                  ...
                  <SuccessPopUp closeAction={() => setShowModel(false)} title={"Team Member Updated Successfully."} showModel={showSuccessPopUp} setShowModel={setShowSuccessPopUp} />
                  {/* error popup */}
